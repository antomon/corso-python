...
2. Creiamo un'istanza di Flask.
3. Utilizziamo il decoratore `@app.route('/')` per definire la route per la homepage alla posizione `/`.
4. Definiamo la funzione `hello_world` che restituisce il testo `Hello, World!`.
5. Avviamo l'applicazione con `app.run(debug=True)`, abilitando la modalità di debug per lo sviluppo.
