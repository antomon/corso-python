...
Un oggetto è considerato **hashable** se ha un valore hash che non cambia durante la sua vita (richiede un metodo `__hash__()`) e può essere confrontato con altri oggetti (richiede un metodo `__eq__()`). Gli oggetti hashable che confrontano uguali devono avere lo stesso valore hash.

La hashability rende un oggetto utilizzabile come chiave in un dizionario o come membro di un set, poiché queste strutture dati usano internamente il valore hash. La maggior parte degli oggetti immutabili incorporati in Python sono hashable; i contenitori mutabili (come liste o dizionari) non lo sono; i contenitori immutabili (come tuple e frozenset) sono hashable solo se i loro elementi sono hashable. Gli oggetti che sono istanze di classi definite dall'utente sono hashable per impostazione predefinita. Confrontano sempre disuguali (tranne che con se stessi), e il loro valore hash è derivato dal loro id().