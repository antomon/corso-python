...
Una stringa in Python è una sequenza di un numero non negativo di caratteri Unicode che, nei corrispondenti letterali, è racchiusa o tra apici singoli `'`, oppure doppi `"`. Per inserire un capo dovrà essere usato `\n` e un singolo backslash alla fine della riga fisica indicherà la continuazione nella riga fisica successiva.

Alternativamente, si possono usare coppie di tripli apici singoli `'''` o doppi  `"""`, dove la differenza è che è possibile inserire un a capo nell'editor. Un singolo backslash non può essere presente.

Nei letterali stringa si possono inserire caratteri non stampabili o caratteri che non sono disponibili a tastiera, usando le cosiddette sequenze di escape (da non confondere con i contenitori discussi nel presente paragrafo), come da tabella seguente:
...
Esistono anche le stringhe _grezze_ (_raw_) sintatticamente identiche alle altre a meno di un suffisso  `r` o  `R` e sono utili quando si vuole evitare che le sequenze di escape siano interpretate. Si usano comunemente per esprimere pattern di espressioni regolari o percorsi di file in Windows. 
...
Per creare una tupla, si può utilizzare una serie di espressioni separate da virgole (`,`), come elementi della tupla. Se ogni elemento è un letterale, l'intera costruzione è un letterale di tupla. Si può opzionalmente mettere una virgola ridondante dopo l'ultimo elemento, che è necessaria se si ha un solo elemento. Si possono raggruppare gli elementi della tupla tra parentesi, ma le parentesi sono necessarie solo quando le virgole avrebbero altrimenti un altro significato (ad esempio, nelle chiamate di funzione), o per denotare tuple vuote o annidate.
...
1. Output: `"Hello"`.
2. Output: `3`.
3. Output: `3`. Concatenazione di tuple che produce una nuova tupla `(42.0, 'Hello', 66, 42)`.
4. Output: `3`. Slicing di tupla che produce una nuova tupla `('Hello', 66)`.
