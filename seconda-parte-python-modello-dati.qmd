...
Gli oggetti di tipi hashable possiedono le seguenti caratteristiche principali:
...
- Accesso e modifica degli elementi:
  ...
  2. Modifica del valore associato alla chiave `"b"`. Output: `{"a": 1, "b": 20, "c": 3}`.
  3. Aggiunta di una nuova coppia chiave-valore. Output: `{"a": 1, "b": 20, "c": 3, "d": 4}`.
...
- Rimozione di elementi:
  ...
  print(valore)  # <3>
  ```
  1. Rimozione della coppia chiave-valore con chiave `"b"` usando `del`. Output: `{"a": 1, "c": 3}`.
  2. Rimozione della coppia chiave-valore con chiave `"c"` usando `pop()`, che restituisce il valore associato. Output: `{"a": 1}`.
  3. Output: `3`.
