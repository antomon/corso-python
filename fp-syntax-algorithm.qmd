...
- Parole chiave: Sono termini riservati del linguaggio che hanno significati specifici e non possono essere utilizzati per altri scopi, come `if`{.python}, `else`{.python}, `while`{.python}, `for`{.python}, ecc.

- Operatori: Simboli utilizzati per eseguire operazioni su identificatori e letterali, come `+`{.python}, `-`{.python}, `*`{.python}, `/`{.python}, `=`{.python}, `==`{.python}, ecc.

- Delimitatori: Caratteri utilizzati per separare elementi del codice, come punto e virgola (`;`{.python}), parentesi tonde (`()`{.python}), parentesi quadre (`[]`{.python}), parentesi graffe (`{}`{.python}), ecc.

- Identificatori: Nomi utilizzati per identificare variabili, funzioni, classi, e altri oggetti.

- Letterali: Rappresentazioni di valori costanti nel codice, come numeri (`123`{.python}), stringhe (`"hello"`{.python}), caratteri (`'a'`{.python}), ecc.
...
- Token coinvolti:
  - `if`{.python}: Parola chiave.
  - `NAME`{.python}: Identificatore.
  - `EQEQUAL`{.python}: Operatore.
  - `NUMBER`{.python}: Letterale numerico.
  - `COLON`{.python}: Delimitatore.

- Lessemi:
  - Il lessema per il token `if`{.python} è la sequenza di caratteri "if".
  - Il lessema per il token `NAME`{.python} è "x".
  - Il lessema per il token `EQEQUAL`{.python} è  "==".
  - Il lessema per il token `NUMBER`{.python} "10".
  - Il lessema per il token `COLON`{.python} ":".

- Pattern:
  - Il pattern per il token `if`{.python} è la stringa esatta "if".
  - Il pattern per un identificatore è una sequenza di lettere e numeri che inizia con una lettera.
  - Il pattern per l'operatore `==`{.python} è la stringa esatta "==".
  - Il pattern per un letterale numerico è una sequenza di cifre.
  - Il pattern per il delimitatore `:`{.python} è la stringa esatta ":".
...
Un'espressione è una combinazione di lessemi che viene valutata per produrre un risultato. Esempi di espressioni includono:
...
- Assegnazione: Utilizza un operatore di assegnazione (ad esempio, `=`) per attribuire un valore a una variabile, che possiamo pensare come un nome simbolico rappresentante una posizione dove è memorizzato un valore. Esempio:
  ...
  x = 5
  ```
  - `x`{.python}: Identificatore della variabile.
  - `=`{.python}: Operatore di assegnazione.
  - `5`{.python}: Letterale numerico intero.
...
- Input/output: Utilizza parole chiave o funzioni di libreria per leggere valori dall'input o scrivere valori all'output. Esempio:
  ...
  print("Hello, World!")
  ```
  - `print`{.python}: Parola chiave o identificatore di funzione di libreria.
  - `"Hello, World!"`{.python}: Letterale stringa.
  L'esecuzione dell'istruzione produce `"Hello, World!"`{.python} in output.
...
- Assegnazione ad espressione: Combinazione di variabili, operatori e valori che producono un risultato assegnato ad una variabile. Esempio:
    ...
    z = (x * 2) + (y / 2)
    ```
  - `z`{.python}: Identificatore della variabile.
  - `=`{.python}: Operatore di assegnazione.
  - `(x * 2)`{.python}: Espressione che moltiplica `x`{.python} per `2`{.python}.
  - `(y / 2)`{.python}: Espressione che divide `y`{.python} per `2`{.python}.
  - `+`{.python}: Operatore aritmetico che somma i risultati delle due espressioni in una più complessa.
  L'esecuzione dell'istruzione produce un risultato valido solo se `x`{.python} e `y`{.python} sono associate a valori numerici e ciò perché non tutte le istruzioni sintatticamente corrette sono semanticamente corrette. D'altronde ciò non deve essere preso come regola, perché se `*`{.python} fosse un operatore che ripete quanto a sinistra un numero di volte definito dal valore di destra e `/`{.python} la divisione del valore di sinistra in parti di numero pari a quanto a destra, allora `x`{.python} e `y`{.python} potrebbero essere stringhe.

### Istruzioni complesse e blocchi di codice

Le **istruzioni complesse** sono costituite da più istruzioni semplici e possono includere strutture di controllo del flusso, come condizioni (`if`{.python}), cicli (`for`{.python}, `while`{.python}) ed eccezioni (`try`{.python}, `catch`{.python}). Queste istruzioni sono utilizzate per organizzare il flusso di esecuzione del programma e possono contenere altre istruzioni semplici o complesse al loro interno.
...
In molti linguaggi di programmazione, i blocchi di codice sono delimitati da parentesi graffe (`{}`{.python}), mentre in altri linguaggi, come Python, l'indentazione è utilizzata per indicare l'inizio e la fine di un blocco di codice.
...
- Esempio in C:
  ...
  ```
  In questo esempio:
  - `if (x > 0)`{.c} è un'istruzione complessa.
  - `{ printf("x è positivo\n"); y = x * 2; }`{.c} è un blocco di codice che viene eseguito se la condizione dell'istruzione `if`{.c} è vera.
...
```python
  ...
  ```
  In questo esempio:
  - `if x > 0:`{.python} è un'istruzione complessa.
  - Le righe indentate sotto l'istruzione `if`{.python} (`print("x è positivo")`{.python} e `y = x * 2`{.python}) costituiscono un blocco di codice che viene eseguito se la condizione dell'istruzione `if`{.python} è vera.
...
- Condizioni: Istruzioni che eseguono un blocco di codice solo se una condizione è vera. Esempio:
  ...
  ```
  - `if`{.c}: Parola chiave che introduce la condizione.
  - `(x > 0)`{.c}: Condizione composta da: `x`{.c} identificatore di variabile, `>`{.c} operatore di confronto e `0`{.c} letterale numerico intero.
  - `{ ... }`{.c}: Delimitatori che racchiudono il blocco di codice.
  - `print("x è positivo")`{.c}: Istruzione di output.
...
- Cicli: Istruzioni che ripetono un blocco di codice. Esempio:
  ...
  ```
  - `for`{.c}: Parola chiave che introduce il ciclo.
  - `(i = 0; i < n; i++)`{.c}: Espressione di controllo del ciclo composta da: `i = 0`{.c} assegnazione iniziale, `i < n`{.c} condizione di ciclo e `i++`{.c} incremento della variabile `i`{.c}.
  - `{ ... }`{.c}: Delimitatori che racchiudono il blocco di codice.
  - `somma = somma + i`{.c}: Operazione aritmetica.
...
- Sintassi:
  - `funzione`{.bash}: Parola chiave che introduce la definizione di una funzione.
  - `calcola_somma`{.bash}: Identificatore della funzione.
  - `(n)`{.bash}: Delimitatori che contengono un identificatore.

- Semantica:
  - Definisce una funzione chiamata `calcola_somma`{.bash} che accetta un parametro `n`{.bash}.
...
- Sintassi:
  - `somma`{.bash}: Identificatore della variabile.
  - `=`{.bash}: Operatore di assegnazione.
  - `0`{.bash}: Letterale numero intero.
...
- Sintassi:
  - `per`{.bash}: Parola chiave che introduce il ciclo.
  - `i`{.bash}: Identificatore della variabile di controllo del ciclo.
  - `da 1 a n`{.bash}: Espressione di controllo del ciclo che indica l'intervallo.

- Semantica:
  - Itera la variabile `i`{.bash} da `1`{.bash} a `n`{.bash}.
...
- Sintassi:
  - `somma`{.bash}: Identificatore della variabile.
  - `=`{.bash}: Operatore di assegnazione.
  - `somma + i`{.bash}: Espressione aritmetica composta da: `somma`{.bash} identificatore di variabile, `+` operatore aritmetico e `i`{.bash} identificatore di variabile.

- Semantica:
  - Aggiunge il valore di `i`{.bash} alla variabile `somma`{.bash} e assegna il risultato a `somma`{.bash}.
...
- Sintassi:
  - `ritorna`{.bash}: Parola chiave che indica la restituzione di un valore.
  - `somma`{.bash}: Identificatore della variabile.

- Semantica:
  - Restituisce il valore della variabile `somma`{.bash} come risultato della funzione.
