...
Nel seguito useremo molto la funzione `print` che permette di visualizzare informazioni sullo schermo. Ciò è fondamentale per capire cosa sta succedendo nel programma in modo immediato. 

Usare `print` è facile: basta scrivere `print()` e mettere nelle parentesi quello che si vuole visualizzare. Ad esempio, per stampare `"Ciao, mondo!"`, si scrive:
...
Una seconda funzione molto utile è `help`, che fornisce informazioni sulla documentazione di moduli, funzioni, classi e metodi. Ad esempio, per ottenere informazioni sulla funzione `print`, possiamo scrivere:

```python
>>> help(print) # <1>
```
1. Output modalità di `help`:
  ```python
...
Nel REPL ha la massima utilità perché eseguendo `help` si passa dalla modalità interattiva del REPL a quella di navigazione del contenuto testuale della risposta. Per ritornare al REPL, basta premere `q` e poi `Invio`.
...
Un esempio è su `@` ove, nel codice seguente, è definita un nuovo tipi di oggetto corrispondente ad una matrice assieme a una delle operazioni matematiche più comuni che è la moltiplicazione, implementata per mezzo di `__matmul__`:
...
5.  Moltiplicazione di matrici utilizzando l'operatore `@` che chiama `__matmul__` per il calcolo effettivo.
6.  Chiama `__repr__` per ottenere la stringa su due righe: `19 22` e `43 50`.
