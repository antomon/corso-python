...
```python
def aggiungi_inverse(dizionario_inverse):  # <1>
  for funzione in list(dizionario_inverse):  # <2>
    dizionario_inverse[dizionario_inverse[funzione]] = funzione  # <3>

math_map = {sin: asin, cos: acos, tan: atan, log: exp}  # <4>
aggiungi_inverse(math_map)  # <5>
```
1. Definizione della funzione `aggiungi_inverse` che prende come parametro un dizionario chiamato `dizionario_inverse`.
2. Itera su una lista delle chiavi del `dizionario_inverse`. La chiamata a `list(dizionario_inverse)` è necessaria per evitare problemi durante la modifica del dizionario stesso durante l'iterazione.
3. Per ogni funzione nel dizionario, aggiunge una nuova coppia chiave-valore invertendo la chiave e il valore originali. In altre parole, per ogni chiave `funzione`, inserisce nel dizionario una nuova chiave che è il valore originale corrispondente a `funzione` e assegna a questa chiave il valore `funzione`.
4. Crea un dizionario `math_map` che mappa alcune funzioni matematiche alle loro funzioni inverse utilizzando il modulo `math`.
5. Chiama la funzione `aggiungi_inverse` passando `math_map` come argomento, completando il dizionario `math_map` con le coppie inverse.

Dopo l'esecuzione, il dizionario `math_map` conterrà sia le funzioni originali che le loro inverse come chiavi e valori:
...
In questo modo, il dizionario `math_map` diventa bidirezionale, mappando ogni funzione alla sua inversa e viceversa.

Notiamo che in questo caso la funzione muta il suo argomento. In Python, la convenzione usuale è che tali funzioni non restituiscano un valore.
...
Quando l'interprete incontra una dichiarazione `def`, compila il corpo della funzione, creando un oggetto funzione, e associa `nome_funzione` all'oggetto funzione compilato nello spazio dei nomi contenitore (tipicamente lo spazio dei nomi del modulo, o uno spazio dei nomi di classe quando si definiscono metodi).
...
I parametri (formalmente, parametri formali) nominano i valori passati in una chiamata di funzione e possono specificare valori predefiniti per essi. Ogni volta che chiamiamo la funzione, la chiamata associa ogni nome di parametro al valore corrispondente in un nuovo spazio dei nomi locale, che Python distrugge successivamente all'uscita dalla funzione.
...
Ecco il codice uniformato per entrambi i casi, con i parametri della funzione diversi ma con lo stesso corpo della funzione:

### Con il marcatore posizionale `/`
...
- **`__name__`**: Questo attributo è l'identificatore che `def` utilizza come nome della funzione. Puoi riassegnare `__name__` a qualsiasi valore stringa, ma se cerchi di dissociarlo, Python genererà un'eccezione `TypeError`.
...
- **`__defaults__`**: Questo attributo contiene una tupla dei valori predefiniti per i parametri nominati della funzione. Puoi riassegnare o dissociare liberamente questo attributo. Se la funzione non ha parametri nominati, `__defaults__` sarà una tupla vuota.
...
Una chiamata deve fornire un argomento per tutti i parametri posizionali e può farlo per i parametri nominati che non sono esclusivamente nominati. La corrispondenza avviene come segue:

1. Gli argomenti nella forma `*espressione` vengono internamente sostituiti da una sequenza di argomenti posizionali ottenuti iterando su `espressione`.

2. Gli argomenti nella forma `**espressione` vengono internamente sostituiti da una sequenza di argomenti nominati i cui nomi e valori sono ottenuti iterando sugli elementi di `espressione`.

3. Diciamo che la funzione ha `N` parametri posizionali e la chiamata ha `M` argomenti posizionali:

    - Quando `M <= N`, tutti gli argomenti posizionali vengono associati ai primi `M` parametri posizionali; i parametri posizionali rimanenti, se presenti, devono essere associati tramite argomenti nominati.

    - Quando `M > N`, i rimanenti argomenti posizionali vengono associati ai parametri nominati nell'ordine in cui appaiono nella firma. Questo processo termina in uno dei seguenti modi:

        1. Tutti gli argomenti posizionali sono stati associati.
        2. Il prossimo elemento nella firma è un collettore di argomenti `*`: l'interprete solleva un'eccezione `TypeError`.
        3. Il prossimo elemento nella firma è un collettore di argomenti `*nome`: gli argomenti posizionali rimanenti vengono raccolti in una tupla che viene quindi associata a `nome` nello spazio dei nomi della chiamata alla funzione.

4. Gli argomenti nominati vengono quindi associati, nell'ordine delle occorrenze degli argomenti nella chiamata, per nome con i parametri, sia posizionali che nominati. I tentativi di riassociare un nome di parametro già associato sollevano un'eccezione `TypeError`.

5. Se a questo punto rimangono argomenti nominati non associati:

    - Quando la firma della funzione include un collettore di argomenti `**nome`, l'interprete crea un dizionario con coppie chiave/valore (nome_argomento, valore_argomento) e lo associa a `nome` nello spazio dei nomi della chiamata alla funzione.

    - In assenza di un tale collettore di argomenti, Python solleva un'eccezione `TypeError`.

6. I parametri nominati rimanenti non associati vengono associati ai loro valori predefiniti.
...
Esempio alternativo con `def`:

```python
def quadrato(valore):  # <1>
  return valore * valore  # <2>

numeri = [5, 2, 8, 3]  # <3>
ordinati = sorted(numeri, key=quadrato)  # <4>
print(ordinati)  # <5>
```
1. Definizione della funzione `quadrato`.
2. La funzione ritorna il quadrato di `valore`.
3. Lista di numeri.
4. Utilizzo della funzione `quadrato` per ordinare la lista.
5. Output: `[2, 3, 5, 8]`.
