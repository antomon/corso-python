...
Scrivere una funzione che crea un file di testo, scrive alcune righe al suo interno e salva il file.
...
def crea_file_di_testo(nome_file):
  with open(nome_file, 'w') as file:  # <1>
    file.write("Prima riga di testo\n")  # <2>
...
```
1. Apre un file in modalità scrittura `w`.
2. Scrive una riga di testo nel file.
3. Scrive un'altra riga di testo nel file.
...
2. Apre un file in modalità scrittura (`w`).
3. Scrive i dati nel file JSON con un'indentazione di 4 spazi.
...
Scrivere una funzione che legge un file JSON e stampa i dati contenuti al suo interno.
...
def leggi_file_json(nome_file):
    with open(nome_file, 'r') as file:  # <2>
        dati = json.load(file)  # <3>
...
```
1. Importa il modulo `json`.
2. Apre un file in modalità lettura (`r`).
3. Legge i dati dal file JSON.
4. Stampa i dati letti dal file.
