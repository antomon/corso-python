...
- File header (`mialibreria.h`):
  ...
  void stampa_messaggio(const char* messaggio, void* risultato, char tipo);
...
- File principale (`main.c`):
  ...
  int main() {
    ...
    stampa_messaggio("Il risultato della somma di interi è: ", &risultato, 'i');

    char* risultato_stringhe = somma_stringhe("Ciao, ", "mondo!"); /* <2> */
    stampa_messaggio("Il risultato della somma di stringhe è: ", risultato_stringhe, 's');
    free(risultato_stringhe); /* <3> */

    int array[] = {1, 2, 3, 4, 5}; 
    int risultato_array = somma_array_interi(array, 5); /* <4> */
    stampa_messaggio("Il risultato della somma dell'array di interi è: ", &risultato_array, 'i');
...
```commonlisp
(defmacro when (test &rest body)  ; <1>
  `(if ,test                      ; <2>
       (progn ,@body)))           ; <3>
```
...
```prolog
genitore(padre, figlio).  % <1>
genitore(madre, figlio).  % <2>
antenato(X, Y) :- genitore(X, Y).  % <3>
antenato(X, Y) :- genitore(X, Z), antenato(Z, Y).  % <4>

?- antenato(padre, figlio).  % <5>
```
...
5. Questa è una query che chiede se `padre` è un antenato di `figlio`.
