...
```
1. Definizione della classe astratta che ha il doppio compito di fornire una implementazione di default per una caratteristica comune (`descrizione`) e un vincolo di implementazione nelle classe derivate per una seconda caratteristica comune non implementabile nello stesso modo per tutte (`faiVerso`).
2. Definizione della classe derivata Cane.
3. Definizione della classe derivata Coccodrillo.

Vediamo come implementare il medesimo esempio con una classe astratta, sempre in in Java, per correggere una chiara incoerenza: `Animale` non può essere un oggetto dato che, in realtà, rappresenta una caratteristica comune di oggetti di tipo animale. 

Quindi, `Animale` diventa una classe astratta, cioè che non può essere istanziata in un oggetto e `Cane` è una classe concreta che la estende ridefinendo il metodo `faiVerso` per comunicare la propria caratteristica distintiva:
...
```
1. Definizione della classe astratta che ha il doppio compito di fornire una implementazione di default per una caratteristica comune (`descrizione`) e un vincolo di implementazione nelle classe derivate per una seconda caratteristica comune non implementabile nello stesso modo per tutte (`faiVerso`).
2. Metodo astratto che restituisce una stringa.
3. Metodo concreto che restituisce una stringa.
