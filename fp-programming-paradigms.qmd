...
- Testo: La sezione `.text`{.bash}  contiene il codice eseguibile. L'etichetta `_start`{.bash}  indica il punto di ingresso del programma.

- Registri: I registri come `al`{.bash}  e `eax`{.bash}  sono utilizzati per operazioni aritmetiche e per la memorizzazione temporanea dei dati.

- Chiamate di sistema: L'interruzione `int 0x80`{.bash}  è usata per eseguire chiamate di sistema in Linux. `int 0x80`{.default} 
...
In questo esempio, la classe `Animale`{.bash} rappresenta una classe base con un attributo `nome`{.bash} e un metodo `faiVerso`{.bash}. La classe `Cane`{.bash} estende `Animale`{.bash} e sovrascrive il metodo `faiVerso`{.bash} per fornire un'implementazione specifica. La classe `Main`{.bash} crea un'istanza di `Cane`{.bash} e chiama il metodo `faiVerso`{.bash}, dimostrando il polimorfismo e l'ereditarietà:
...
Vediamo come implementare il medesimo esempio con una classe astratta in Java. `Animale`{.bash} è la classe astratta, cioè che non può essere istanziata in un oggetto e `Cane`{.bash} è una classe concreta che la estende:
...
Nel codice sotto proposto è definita la macro `when`{.bash}, che prende due parametri in input, cioè `test`{.bash} e `body`{.bash}, ove `test`{.bash} è un'espressione condizionale e `body`{.bash} un insieme di istruzioni da eeseguire se la condizione è vera:
...
Vediamo un esempio pratico di come si utilizza la macro `when`{.bash}. Il test è valutare se `x`{.bash} è maggiore di `10`{.bash} e, nel caso, stampare `"x is greater than 10"`{.bash} e poi assegnare `x`{.bash} a `0`{.bash}. Chiamiamo la macro con i due parametri:
...
Di seguito due funzioni, la prima `sumToN`{.bash} è pura e somma i primi `n`{.bash} numeri. `(*2)`{.bash} è una funzione che prende un argomento e lo moltiplica per 2 e ciò rende la seconda funzione `applyFunction`{.bash} una vera funzione di ordine superiore, poiché accetta `(*2)`{.bash} come argomento oltre ad una lista, producendo come risultato il raddoppio di tutti i suoi elementi:
