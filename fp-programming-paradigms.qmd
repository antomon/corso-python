...
- Dati: La sezione `{nasm} .data`è utilizzata per dichiarare variabili statiche inizializzate.

- Testo: La sezione `{nasm} .text`  contiene il codice eseguibile. L'etichetta `_start`{.bash}  indica il punto di ingresso del programma.

- Registri: I registri come `{nasm} al`  e `{nasm} eax`  sono utilizzati per operazioni aritmetiche e per la memorizzazione temporanea dei dati.

- Chiamate di sistema: L'interruzione `{nasm} int 0x80`  è usata per eseguire chiamate di sistema in Linux.
