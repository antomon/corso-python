...
- Dati: La sezione `.data`{.bash} è utilizzata per dichiarare variabili statiche inizializzate.

- Testo: La sezione `.text`{.bash}  contiene il codice eseguibile. L'etichetta `_start`{.nasm}  indica il punto di ingresso del programma.

- Registri: I registri come `al`{.nasm}  e `eax`{.nasm}  sono utilizzati per operazioni aritmetiche e per la memorizzazione temporanea dei dati.

- Chiamate di sistema: L'interruzione `int 0x80`{.nasm}  è usata per eseguire chiamate di sistema in Linux.
...
- Funzione: Una funzione è un blocco di codice che esegue un compito specifico e restituisce un valore. Le funzioni sono utilizzate per calcoli o operazioni che producono un risultato. Ad esempio, una funzione che calcola la somma di due numeri:
...
- Procedura: Una procedura è simile a una funzione, ma non restituisce un valore. È utilizzata per eseguire azioni o operazioni che non necessitano di un risultato. Ad esempio, una procedura che stampa un messaggio:
...
Ad esempio, definiamo la classe `Box`{.cpp} nel modo seguente:
...
Essa può contenere un valore di qualsiasi tipo specificato al momento della creazione dell'istanza per mezzo del template `T`{.cpp}:
...
Nel codice sotto proposto è definita la macro `when`{.commonlisp}, che prende due parametri in input, cioè `test`{.commonlisp} e `body`{.commonlisp}, ove `test`{.commonlisp} è un'espressione condizionale e `body`{.commonlisp} un insieme di istruzioni da eeseguire se la condizione è vera:
...
Vediamo un esempio pratico di come si utilizza la macro `when`{.commonlisp}. Il test è valutare se `x`{.commonlisp} è maggiore di `10`{.commonlisp} e, nel caso, stampare `"x is greater than 10"`{.commonlisp} e poi assegnare `x`{.commonlisp} a `0`{.commonlisp}. Chiamiamo la macro con i due parametri:
...
In questo esempio, la classe `Animale`{.java} rappresenta una classe base con un attributo `nome`{.java} e un metodo `faiVerso`{.java}. La classe `Cane`{.java} estende `Animale`{.java} e sovrascrive il metodo `faiVerso`{.java} per fornire un'implementazione specifica. La classe `Main`{.java} crea un'istanza di `Cane`{.java} e chiama il metodo `faiVerso`{.java}, dimostrando il polimorfismo e l'ereditarietà:
...
Vediamo come implementare il medesimo esempio con una classe astratta in Java. `Animale`{.java} è la classe astratta, cioè che non può essere istanziata in un oggetto e `Cane`{.java} è una classe concreta che la estende:
...
Di seguito due funzioni, la prima `sumToN`{.haskell} è pura e somma i primi `n`{.haskell} numeri. `(*2)`{.haskell} è una funzione che prende un argomento e lo moltiplica per 2 e ciò rende la seconda funzione `applyFunction`{.haskell} una vera funzione di ordine superiore, poiché accetta `(*2)`{.haskell} come argomento oltre ad una lista, producendo come risultato il raddoppio di tutti i suoi elementi:
