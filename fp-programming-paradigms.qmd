...
- File di implementazione (`mialibreria.c`):
  ...
  char* somma_stringhe(const char* a, const char* b) {
      ...
      // +1 per il carattere di terminazione '\0'
...
- File principale (`main.c`):
  ...
  int main() {
      ...
      printf("%s\n", risultato_stringhe);
      free(risultato_stringhe); // Libera la memoria allocata
...
Vediamo come implementare il medesimo esempio con una classe astratta, sempre in in Java, per correggere una chiara incoerenza: `Animale` non può essere un oggetto dato che, in realtà, è una caratteristica comune di oggetti di tipo animale. 
