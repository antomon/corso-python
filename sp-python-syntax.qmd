...
| Assegnazione | `:=`{.python} | Operatore di assegnazione in espressione (walrus o tricheco)                      |
...
- Interi, possono essere scritti in base decimale, ottale, esadecimale o binaria:
  - Decimale: `10`{.python}, `-3`{.python}.
  - Ottale: `0o12`{.python}, `-0o7`{.python}.
  - Esadecimale: `0xA`{.python}, `-0x1F`{.python}.
  - Binario: `0b1010`{.python}, `-0b11`{.python}.
- Virgola mobile, possono essere rappresentati con una parte intera e una decimale, oppure con notazione scientifica:
  - Virgola mobile: `3.14`{.python}, `-0.001`{.python}.
  - Notazione scientifica: `1e10`{.python}, `-2.5e-3`{.python}.
- Complessi, appresentati da una parte reale e una parte immaginaria: `3+4j`{.python}, `-1-0.5j`{.python}.
...
- Stringhe multi-linea racchiuse tra triple virgolette singole o doppie:
  - Triple singole: `'''testo multi-linea'''`{.python}.
  - Triple doppie: `"""testo multi-linea"""`{.python}.
...
Tutte le stringhe sono codificate in Unicode, con il prefisso `b`{.python} la stringa è di tipo byte ed è limitata ai 128 caratteri dell'ASCII. Se si prepone `r`{.python}, che sta per _raw_ cioè grezzo, allora la codifica è sempre Unicode ma i caratteri di escape non sono intepretati.[2^]

[2^]:

#### F-stringhe

Le f-stringhe (stringhe formattate) sono racchiuse tra virgolette singole, doppie o triple e sono precedute dal prefisso `f`{.python} o `F`{.python}. Permettono di includere espressioni Python all'interno.
...
Un'istruzione semplice è sempre contenuta in una riga logica, che può presentare più istruzioni semplici separate da `;`{.python}. È permesso ma scaonsigliato perché in pochi casi porta a codice leggibile.
...
Un assegnamento con `=`{.python} è anch'esso un'istruzione semplice e non può mai essere all'interno di una espressione, dove invece si può usare l'operatore _tricheco_ `:=`{.python}:
