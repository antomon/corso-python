...
- `NomeClasse` è un identificatore: una variabile che la dichiarazione `class`, quando completata, associa (o riassocia) all'oggetto classe appena creato. Le convenzioni di denominazione Python consigliano di utilizzare il maiuscolo per i nomi delle classi, come `Item`, `PrivilegedUser`, `MultiUseFacility`, ecc.

- `classi-base` è una serie di espressioni delimitate da virgole i cui valori sono oggetti classe. Vari linguaggi di programmazione utilizzano nomi diversi per questi oggetti classe: possiamo chiamarli basi, superclasse o genitori della classe. Possiamo dire che la classe creata eredita da, deriva da, estende o sottoclasse le sue classi base; in questo capitolo, generalmente usiamo il termine estendere. Questa classe è una sottoclasse diretta o discendente delle sue classi base. `**kw` può includere un argomento denominato `metaclass` per stabilire la metaclasse della classe.
...
10. Chiamata della funzione `di_ciao` decorata.

## Spazio dei nomi

I parametri di una funzione, insieme a qualsiasi nome che viene associato (mediante assegnazione o altre istruzioni di associazione, come `def`) nel corpo della funzione, costituiscono lo spazio dei nomi locale della funzione, noto anche come ambito locale. Ognuna di queste variabili è nota come variabile locale della funzione.

Le variabili che non sono locali sono note come variabili globali. Le variabili globali sono attributi dell'oggetto modulo. Ogni volta che una variabile locale ha lo stesso nome di una variabile globale, quel nome, all'interno del corpo della funzione, si riferisce alla variabile locale, non a quella globale. Espressiamo questo dicendo che la variabile locale nasconde la variabile globale dello stesso nome in tutto il corpo della funzione.

### `global`

Per impostazione predefinita, qualsiasi variabile associata nel corpo di una funzione è locale alla funzione. Se una funzione ha bisogno di associare o riassociare alcune variabili globali (non è una buona pratica!), la prima istruzione del corpo della funzione deve essere:

```python
global identificatori
```

dove `identificatori` è uno o più identificatori separati da virgole. Gli identificatori elencati in una dichiarazione `global` si riferiscono alle variabili globali (cioè, attributi dell'oggetto modulo) che la funzione ha bisogno di associare o riassociare.
...
print(contatore())  # <7>
```
1. Definizione di una variabile globale `_conta`.
2. Definizione della funzione `contatore`.
3. Dichiarazione che `_conta` è una variabile globale.
4. Incremento della variabile globale `_conta`.
5. Restituzione del valore di `_conta`.
6. Chiamata della funzione `contatore`. Output: 1.
7. Chiamata della funzione `contatore`. Output: 2.

### Funzioni Annidate e Ambiti Annidati

Un'istruzione `def` all'interno di un corpo di funzione definisce una funzione annidata e la funzione il cui corpo include il `def` è nota come funzione esterna a quella annidata. Il codice nel corpo di una funzione annidata può accedere (ma non riassociare) variabili locali di una funzione esterna, note anche come variabili libere della funzione annidata.
...
esterna(10, 20, 30)  # <5>
```
1. Definizione della funzione esterna `esterna` con i parametri `a`, `b` e `c`.
2. Definizione della funzione annidata `annidata` con il parametro `x`.
3. Calcolo e restituzione della percentuale di `x` rispetto alla somma di `a`, `b` e `c`.
4. Stampa delle percentuali di `a`, `b` e `c` calcolate dalla funzione annidata.
5. Chiamata della funzione esterna `esterna`.

### Espressioni `lambda`

Se il corpo di una funzione è una singola istruzione di espressione return, possiamo (opzionalmente) scegliere di sostituire la funzione con una forma speciale di espressione lambda:

```python
lambda parametri: espressione
```

Un'espressione lambda è l'equivalente anonimo di una funzione normale il cui corpo è una singola istruzione di return. La sintassi lambda non utilizza la parola chiave return. Possiamo utilizzare un'espressione lambda ovunque potremmo utilizzare un riferimento a una funzione. Le espressioni lambda possono essere utili quando vogliamo utilizzare una funzione estremamente semplice come argomento o valore di ritorno.

Esempio di espressione lambda:

```python
lista = [-2, -1, 0, 1, 2]  # <1>
ordinata = sorted(lista, key=lambda x: x * x)  # <2>
print(ordinata)  # <3>
```
1. Definizione di una lista di numeri.
2. Ordinamento della lista utilizzando una funzione lambda che restituisce il quadrato di ciascun elemento come chiave di ordinamento.
3. Stampa della lista ordinata. Output: [0, -1, 1, -2, 2].

In alternativa, possiamo sempre utilizzare una dichiarazione `def` locale per dare un nome all'oggetto funzione, quindi utilizzare questo nome come argomento o valore di ritorno.

Esempio alternativo senza `lambda`:
...
lista = [-2, -1, 0, 1, 2]  # <3>
ordinata = sorted(lista, key=quadrato)  # <4>
print(ordinata)  # <5>
```
1. Definizione della funzione `quadrato` con il parametro `valore`.
2. Calcolo e restituzione del quadrato del valore.
3. Definizione di una lista di numeri.
4. Ordinamento della lista utilizzando la funzione `quadrato` come chiave di ordinamento.
5. Stampa della lista ordinata. Output: [0, -1, 1, -2, 2].

Mentre `lambda` può essere utile in alcuni casi, `def` è di solito preferibile: è più generale e aiuta a rendere il nostro codice più leggibile, poiché possiamo scegliere un nome chiaro per la funzione.
